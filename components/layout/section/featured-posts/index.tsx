        ...
        <div role={'featured posts container'} className={`grid grid-flow-col auto-cols-max place-content-center`}>
          {
            posts.map(({createdOn, imageURL, categories, title, summary}) => (
              <div role={'featured post'} className={'m-4 max-w-[18rem] min-w-[18rem] mx-6 rounded-xl hover:-mt-2 hover:ease-in-out hover:delay-75'}>
                <img role={'featured post image'} src={`${imageURL}`} alt="post image" className={`rounded-t-xl min-h-[13rem] max-h-[13rem] bg-cover bg-center object-cover`}/>
